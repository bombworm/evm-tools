...
const useCallFunction = (args, types, fn, opts) => {
  ...
  const callFunction = async () => {
    ...
    if (fn.stateMutability !== "view" && fn.constant !== true && !opts.callStatic) {
      ...
      let viewOpts = Object.fromEntries(Object.entries(opts).filter(([key]) => key == "blockTag"))
      if (!isNaN(opts["blockTag"])) {
        viewOpts["blockTag"] = Number(opts["blockTag"])
      }
      // view fn
      const result = await instance.callStatic[fn.name](...processedArgs, viewOpts);

      // simple return type
      if (!Array.isArray(result)) {
        if (opts["blockTag"]) {
          addLogItem(`block(${opts["blockTag"]}):` + result.toString());
        } else {
